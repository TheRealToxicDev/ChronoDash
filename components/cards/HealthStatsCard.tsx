...
import { FiCpu, FiHardDrive, FiClock, FiActivity, FiTrendingUp } from "react-icons/fi";
...
export default function HealthStats({ health, loading, error }: HealthStatsProps) {
  if (loading) {
    return (
      <div className="grid grid-cols-2 gap-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-background/50 rounded-xl p-4 animate-pulse border border-border/50">
  ...
  const formatUptime = (uptime: string) => {
    try {
      return formatDistanceToNow(new Date(health.startTime), { addSuffix: false });
    } catch (e) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      {stats.map((stat, index) => (
        ...
        >
          {/* Background gradient */}
          <div className={`absolute inset-0 bg-gradient-to-br ${stat.gradient} opacity-30`}></div>
